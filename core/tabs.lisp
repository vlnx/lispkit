...
(defcallback notebook-page-removed :void
  ...
  (dmesg "page-removed")
  (let ((browser (find-instance 'of-browser 'from-notebook notebook))
    ...
    ;; possibly changed by removing this page
...
(defun tab-new (browser uri &key background)
  ...
  (let (tab new-index)
    (cond ((current-tab-is-blank-p browser) ; Use the blank tab
...
(defun tab-remove (browser tab)
  ...
  (unless (member tab (browser-tabs browser))
    (error "tab is not registered to that browser instance"))
